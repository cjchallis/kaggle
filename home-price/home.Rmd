...
```{r handle_na}
factors = c("Alley", "Fence", "MiscFeature", "Electrical")
notAlwaysApplicable = c("WoodDeckSF", "LotFrontage")
varGroups = c("Bsmt", "Garage", "Fireplace", "Pool", "MasVnr")
repVars = c("BsmtQual", "GarageQual", "FireplaceQu", "PoolQC", "MasVnrType")
...
There are a few problem variables we need to take care of. First, `MSSubClass` should be a factor, but its levels are given as integers so it is read as a numeric variable.
```{r MSSubClass}
train$MSSubClass = as.factor(train$MSSubClass)
test$MSSubClass = as.factor(test$MSSubClass)
```

There are also a lot of "NA" values throughout the data set. For most of the variables, this means that the field is not applicable to the house, like basement square footage when there is no basement. So for the factor variables this should be a separate factor.



In addition, many of the character variables have "NA" values. I assume that in this data set that means the variable does not apply, not that the value is missing. I prefer to treat "NA" has a separate factor level as a starting point for all variables.

```{r histogram}
plot(hist(train$SalePrice))
```

Unsurprisingly, the sale price distribution looks log-normal. We also have the benefit of knowing that the competition is judged on root mean squared logarithmic error.
...
A linear model can be a good part of data exploration.
...
training_no_singular = training %>% select(-one_of(singular))
lm_model <- lm(LogSalePrice ~ ., data=training_no_singular)
summary(lm_model)
```
